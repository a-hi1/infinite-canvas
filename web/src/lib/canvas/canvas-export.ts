...
/** Export selected canvas nodes as media files / txt / json zip (local-only, no cloud). */
export async function exportCanvasNodes(nodes: CanvasNodeData[], fileName = "画布元素") {
    if (!nodes.length) throw new Error("没有可导出的节点");
    const zipFiles: { name: string; data: BlobPart }[] = [];
    ...
    if (!zipFiles.length) throw new Error("没有可导出的内容");
