            ...
            saveSource: (source) =>
                set((state) => ({
                    sources: state.sources.some((item) => item.id === source.id)
                        ? state.sources.map((item) => (item.id === source.id && !item.builtIn ? createPromptSource(source) : item.builtIn && item.id === source.id ? { ...item, enabled: source.enabled } : item))
        ...
        }),
        {
            ...
            merge: (persisted, current) => {
                ...
                const builtIn = DEFAULT_PROMPT_SOURCES.map((source) => ({
                    ...source,
                    enabled: enabledById.has(source.id) ? Boolean(enabledById.get(source.id)) : source.enabled,
                }));
