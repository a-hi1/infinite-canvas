                ...
                <header className="sticky top-0 z-20 h-16 shrink-0 border-b border-stone-200 bg-background/90 backdrop-blur-xl dark:border-stone-800">
                    <div className="mx-auto flex h-full max-w-7xl items-stretch justify-between gap-5 px-6">
                            ...
                            <nav className="hide-scrollbar ml-8 hidden h-16 min-w-0 items-center gap-7 overflow-x-auto md:flex">
                                {navigationTools.map((tool) => {
                                    ...
                                    if ("action" in tool && tool.action === "open-config") {
                                        return (
                                            <button key={tool.slug} type="button" className={itemClass} onClick={() => openConfigDialog(false)} aria-label="配置" title="配置">
                        ...
                        <div className="my-auto flex h-9 min-w-0 items-center justify-end gap-2 justify-self-end whitespace-nowrap">
                            {/* 主导航保留「配置」文案入口；右侧齿轮与原先一致，仍可打开配置弹窗 */}
                            <UserStatusActions showConfig />
                        </div>
                    </div>
                </header>
            ) : null}
