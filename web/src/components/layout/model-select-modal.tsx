...
import { isSameOriginRelayBaseUrl, type ModelChannel } from "@/stores/use-config-store";

/** 选择渠道模型：拉取上游列表或手动增加，勾选后才会进入渠道模型列表。 */
export function ModelSelectModal({
    ...
    const fetchModels = async () => {
        ...
        if (!channel.baseUrl.trim() || (!channel.apiKey.trim() && !isSameOriginRelayBaseUrl(channel.baseUrl))) {
            message.error("请先填写接口地址和 API Key；服务器代理/内网中继未要求令牌时 API Key 可留空");
            return;
        }
        setLoading(true);
        try {
            const models = withSoraRelayModelAliases(await fetchChannelModels(channel));
            setFetched(models);
            setActiveTab("new");
            ...
            message.success(
                injectedAzure && models.some((item) => /^sora([-_.]|$)|sora-2/i.test(item))
                    ? `已拉取 ${models.length} 个模型（含本地补全的 azure-sora）`
    ...
    const confirm = () => {
        ...
        onConfirm(withSoraRelayModelAliases(ordered));
        onClose();
    };
    ...
    return (
        ...
        >
            ...
            <div className="mt-2 text-xs text-stone-500">如果上游不提供 OpenAI /models 接口，请手动增加模型名称。Sora 相关会本地补 azure-sora 别名。</div>
