...
+ [调整] 登录账号弹层始终展示云端积分余额；平台代生成是否就绪仅改文案，不再整块隐藏余额。
+ [新增] 清单实现核验文档：区分「代码已实现」与「仍须人工点验」。
+ [新增] 可选模型调用脚本：按 `渠道::模型` 本地配置；空脚本走系统默认；工作台/画布/ModelPicker 可识别脚本模式。
+ [新增] 画布顶栏导出：导出当前画布 zip、导出选中节点 zip（不引入完整上游侧栏）。
+ [修复] 画布透明背景开关可稳定开/关；节点 AI 配置合并收口，避免字段漏合并。
+ [调整] P0.5c 本机自动验收快照写入验收清单；主线改为「代码已齐 + 人工验收收口」，完整侧栏/资产 Tab 明确后置。
+ [新增] 画布组节点：可创建组、拖入节点分组、拖动组带动子节点；组不可连线。
+ [调整] 画布库 / 素材库上云状态标识更清晰。
+ [修复] 画布云同步 tombstone，避免删除项目同步复活。
+ [新增] P2.0-A/B/C：画布 JSON、媒体 blob、素材 manifest 本地优先云同步。
+ [新增] 上游跟进矩阵与双轨冒烟脚本。
+ [新增] 图像设置「透明背景」开关（BYOK，`background=transparent`）。
+ [调整] 平台扣积分/支付产品面后置；默认 BYOK。
+ [新增] 可选云端账号与生成历史上云、`ai-proxy`、P0.5c 验收与健康检查。
+ [新增] Codex App 插件支持。
+ [调整] Docker 改为 nginx 静态托管；前端 Vite 构建。
