...
画布与素材**默认本地优先**；登录后可选同步到云端（见 `/api/projects`、`/api/blobs`、`/api/assets`），云失败不丢本地稿。
...
画布 JSON 不直接长期保存大体积 base64 图片或视频。图片节点、视频节点、助手图片和素材媒体只保存展示 URL、`storageKey` 和元信息，真实 Blob 通过 `storageKey` 读取；上云时媒体走 blob 接口，清单/项目 JSON 单独推送。
