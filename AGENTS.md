...
- 用户要求「了解项目」「接管上下文」「继续开发」或新对话需要快速上手时：**先读** `docs/content/docs/progress/session-handoff.mdx`，再按其中清单只读核对 `git` 与 progress 文档；**不要**跳过接手文档直接改代码。
- **后续实质开发必须同步文档**（有变化才改，禁止空改）：
  - 三套部署 / remote / 硬约束 / 主线里程碑 / 未推风险 / 接手汇报清单 → 更新 `session-handoff.mdx`
  - 长期架构与端口默认摘要 → `CLAUDE.md`
  - 可执行开发规则、反复踩坑 → 本文件 `AGENTS.md`
  - 待办 → `docs/content/docs/progress/todo.mdx`
  - 已实现待人工点验 → `docs/content/docs/progress/pending-test.mdx`
  - 上游切片 → `docs/content/docs/progress/upstream-follow.mdx`
  - 运维最短命令 → `docs/content/docs/progress/ops-daily.mdx`
  - 工作空间产品说明 → `docs/content/docs/overview/workspace.mdx`
  - 云 API 行为 → `docs/content/docs/overview/cloud-api.mdx`
- 功能合入或会话收尾前自检：只读 `session-handoff` + `git status` 能否 5 分钟内知道「能推什么、部署哪台、别踩什么」。
- 不要记录、复述或提交真实 API Key / FRP token / `.env` 内容。
...
- 不要为了自检而跑无关的全量类型检查；用户未要求时，不必为「检查语法」单独开一轮 tsc。
- **本机 Docker 自动重启（强制）：** 改动会影响本机 Docker 里正在跑的前端/API/代理时，在改完并同步必要文档后，**主动**执行重建重启，**不要等用户再说「重启」**：
  - 仅前端：`docker compose -f docker-compose.local.yml up -d --build app`
  - 含 `api/`：同上并带上 `api`（例：`up -d --build app api`）
  - 含 `ai-proxy/`：一并 `--build` 对应服务
  - 完成后用本机 `http://127.0.0.1:3011/`（及需要时 `/api/health`）做一次健康检查；回复里简短说明已重启 + 建议硬刷新
  - **纯文档 / 记忆 / 未进镜像的文件**可不重启
  - **不要**自动部署美国服务器或公司内网机（用户自部署）；公司机仍须 `-p ic3011` 且不碰其它容器
- 不要改无关文件，不要顺手重构。
...
- 若注意事项属于「部署环境差异 / 接手清单 / 禁止操作」类，**同时**更新 `session-handoff.mdx`，避免只写在 AGENTS 而新会话读不全。
...
- 画布项目与“我的素材”默认保存在浏览器本地；登录后可选云同步（项目 JSON、画布媒体 blob、素材 manifest，本地优先，云失败不丢稿）。文档应写「本地优先 + 可选云同步」，不要写成仅本地且永不云同步，也不要写成强制云端 SaaS。
