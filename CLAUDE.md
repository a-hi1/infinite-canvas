...
**P1.0-A 已落地：** `api` 已把 JSON 数据访问按 `users / sessions / jobs / files` 拆成 repository façade，底层暂不换库、不改接口和存储格式；目的是为后续 Postgres / 账本 / 项目同步铺路，减少再次从 `index.js` 里拆逻辑。下一步更适合继续收口领域常量与状态枚举，而不是立刻跳到支付或画布全量同步。 
