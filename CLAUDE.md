...
**P0.5c 当前主线（验稳收口，非新功能浪潮）：** 双轨回归 + 本机/服务器环境分册 + vidgen 预期写清后再开 P1/P2。清单见 `docs/content/docs/progress/p05c-acceptance.mdx`；健康检查 `scripts/check-cloud-stack.sh` / `scripts/check-cloud-stack.ps1`。本机上云占本机 `./data/api`，服务器上云占服务器 `./data/api`，互不影响。
