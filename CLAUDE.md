...
**P1.0-D / P1.0-E / P1.0-F 已落地：** 可选平台图片网关 `POST /api/generate/image`（文生图 + 最多 4 张参考图 edits）与最小平台视频网关 `POST /api/generate/video`（仅 OpenAI 兼容文生视频）。均须对应 `API_PLATFORM_*_ENABLED=true` + 上游配置；成功后共用 `platform-billing` 落盘/幂等扣费。`/auth/me.credits` 分字段 `platform_image_enabled` / `platform_video_enabled`。工作台各自可选开关，默认 BYOK。支付入账仍后置；平台视频暂无参考素材/Grok/Seedance 服务端路径。 
